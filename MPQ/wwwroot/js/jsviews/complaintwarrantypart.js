...
function ListarTodos() {
    ...
    $.ajax({ type: "GET", url: "/ComplaintWarrantyPart/GetAllComplaintWarrantyPart", dataType: "json" })
        .done(function (data) {

            _registros = data;

            var source = { datatype: "json", datafields: [{ name: 'id' }, { name: 'site' }, { name: 'customer' }, { name: 'receiptDate' }, { name: 'partQuantity' }, { name: 'issue' }, { name: 'legitimate' }, { name: 'status' }, { name: 'businessUnit' }], localdata: _registros };
            var dataAdapter = new $.jqx.dataAdapter(source);
            var _c = tables.complaintwarrantypart;
            ...
            _jqxGrid = $("#jqxGrid").jqxGrid({
                ...
                columns: [
                            ...
                            button.click(function (event) {
                                ...
                                if (IndexGrid != undefined && IndexGrid > -1) {
                                    ...
                                    if (reg != undefined && reg != null) {
                                        ...
                                        $.ajax({ type: 'GET', url: `/ComplaintWarrantyPart/Update?registro=${reg.id}`, contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
                    ...
                    {
                        text: _c.site, datafield: 'site', width: '150px', cellsalign: 'left'
                    },
                    {
                        text: _c.receiptDate, datafield: 'receiptDate', width: '100px', cellsalign: 'left'
                    },
                    {
                        text: _c.customer, datafield: 'customer', width: '200px', cellsalign: 'left'
                    },
                    {
                        text: _c.partQuantity, datafield: 'partQuantity', width: '100px', cellsalign: 'right'
                    },
                    {
...
function Carregar() {
    $('#botaoCriar').on('click', function () {
        $.ajax({ type: 'GET', url: '/ComplaintWarrantyPart/Create', contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
...
function Deletar(complaintwarrantypart) {

    $.ajax({ type: 'POST', url: '/ComplaintWarrantyPart/Delete', dataType: 'json', data: { registro: complaintwarrantypart.id, } })
        .done(function (data) {
