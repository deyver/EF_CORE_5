...
function ListarTodos() {
    ...
    $.ajax({ type: "GET", url: "/GoalStf/GetAllGoalStf", dataType: "json" })
        .done(function (data) {

            _registros = data;

            var source = { datatype: "json", datafields: [{ name: 'id' }, { name: 'site' }, { name: 'indicator' }, { name: 'businessUnit' }, { name: 'goal' }, { name: 'stfNumber' }, { name: 'generalTarget' }], localdata: _registros };
            var dataAdapter = new $.jqx.dataAdapter(source);
            var _c = tables.goalstf;
            ...
            _jqxGrid = $("#jqxGrid").jqxGrid({
                ...
                columns: [
                            ...
                            button.click(function (event) {
                                ...
                                if (IndexGrid != undefined && IndexGrid > -1) {
                                    ...
                                    if (reg != undefined && reg != null) {
                                        ...
                                        $.ajax({ type: 'GET', url: `/GoalStf/Update?registro=${reg.id}`, contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
                    ...
                    {
                        text: _c.indicator, datafield: 'indicator', width: '200px', cellsalign: 'left'
                    },
                    {
                        text: _c.businessunit, datafield: 'businessUnit', width: '200px', cellsalign: 'left'
                    },
                    {
                        text: _c.goal, datafield: 'goal', width: 'auto', cellsalign: 'left'
                    },
                    {
                        text: _c.stfnumber, datafield: 'stfNumber', width: '50px', cellsalign: 'right'
                    },
                    {
                        text: _c.generaltarget, datafield: 'generalTarget', width: '150px', cellsalign: 'right'
                    }
                ],
...
function Carregar() {
    $('#botaoCriar').on('click', function () {
        $.ajax({ type: 'GET', url: '/GoalStf/Create', contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
...
function Deletar(goalstf) {

    $.ajax({ type: 'POST', url: '/GoalStf/Delete', dataType: 'json', data: { registro: goalstf.id, } })
        .done(function (data) {
