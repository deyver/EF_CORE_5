...
function ListarTodos() {
    ...
    $.ajax({ type: "GET", url: "/Grupo/GetAllGroup", dataType: "json" })
        .done(function (data) {
        
            _registros = data;

            var source = { datatype: "json", datafields: [{ name: 'id' }, { name: 'name' }, { name: 'description' }], localdata: _registros };
        ...
        _jqxGrid = $("#jqxGrid").jqxGrid({
            ...
            columns: [
                        ...
                        button.click(function (event) {
                            ...
                            if (IndexGrid != undefined && IndexGrid > -1) {
                                ...
                                if (reg != undefined && reg != null) {
                                    ...
                                    $.ajax({ type: 'GET', url: `/Grupo/Update?registro=${reg.id}&dominio=${reg.login}`, contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
                ...
                {
                    text: _c.nome, datafield: 'name', width: '150px', cellsalign: 'left'
                },
                {
                    text: _c.descricao, datafield: 'description', width: 'auto', cellsalign: 'left'
                }
            ],
...
function Carregar() {
    $('#botaoCriar').on('click', function () {
        $.ajax({ type: 'GET', url: '/Grupo/Create', contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
    ...
    //Botao Exportar
    $("#btnExportar").click(function () { $("#jqxGrid").jqxGrid('exportdata', 'xls', 'Grupos', true, null, false); });
}

function Deletar(grupo) {
    $.ajax({ type: 'POST', url: '/Grupo/Delete', dataType: 'json', data: { registro: grupo.id } })
        .done(function (data) {
