...
function ListarTodos() {
    ...
    $.ajax({ type: "GET", url: "/ProducedPart/GetAll", dataType: "json" })
        .done(function (data) {
        ...
        var source = { datatype: "json", datafields: [{ name: 'id' }, { name: 'site' }, { name: 'productionarea' }, { name: 'year' }, { name: 'weeknumber' }, { name: 'quantity' } ], localdata: _registros };
        var dataAdapter = new $.jqx.dataAdapter(source);

        var _c = tables.producedpart;
        ...
        _jqxGrid = $("#jqxGrid").jqxGrid({
            ...
            columns: [
                        ...
                        button.click(function (event) {
                            ...
                            if (IndexGrid != undefined && IndexGrid > -1) {
                                ...
                                if (reg != undefined && reg != null) {
                                    ...
                                    $.ajax({ type: 'GET', url: `/ProducedPart/Update?id=${reg.id}`, contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
                ...
                {
                    text: _c.siteid, datafield: 'site', width: 'auto'
                },
                {
                    text: _c.productionareaid, datafield: 'productionarea', width: 'auto'
                },
                {
                    text: _c.year, datafield: 'year', width: 'auto'
                },
                {
                    text: _c.weeknumber, datafield: 'weeknumber', width: 'auto'
                },
                {
                    text: _c.quantity, datafield: 'quantity', width: 'auto'
                }
            ],
        ...
        $('#jqxGrid').jqxGrid('hidecolumn', 'idIdiomaPadrao');
...
function Carregar() {
    ...
    $('#botaoCriar').on('click', function () {
        $.ajax({ type: 'GET', url: '/ProducedPart/Create', contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
    ...
    //Botao Exportar
    $("#btnExportar").click(function () { $("#jqxGrid").jqxGrid('exportdata', 'xls', 'ProducedPart', true, null, false); });
}

function Deletar(producedPart) {
    
    $.ajax({ type: 'POST', url: '/ProducedPart/Delete', dataType: 'json', data: { id: producedPart.id } })
        .done(function (data) {
