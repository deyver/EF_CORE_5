...
function ListarTodos() {
    ...
    $.ajax({ type: "GET", url: "/Indicador/GetAllIndicator", dataType: "json" })
        .done(function (data) {
        
        _registros = data;

        var source = { datatype: "json", datafields: [ { name: 'id' }, { name: 'name' } ], localdata: _registros };
            var dataAdapter = new $.jqx.dataAdapter(source);
            var _c = tables.indicador;
        ...
        _jqxGrid = $("#jqxGrid").jqxGrid({
            ...
            columns: [
                        ...
                        button.click(function (event) {
                            ...
                            if (IndexGrid != undefined && IndexGrid > -1) {
                                ...
                                if (reg != undefined && reg != null) {
                                    ...
                                    $.ajax({ type: 'GET', url: `/Indicador/Update?registro=${reg.id}`, contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
                ...
                {
                    text: _c.nome, datafield: 'name', width: 'auto'
                }
            ],
...
function Carregar() {
    ...
    $('#botaoCriar').on('click', function () {
        $.ajax({ type: 'GET', url: '/Indicador/Create', contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
    ...
    //Botao Exportar
    $("#btnExportar").click(function () { $("#jqxGrid").jqxGrid('exportdata', 'xls', 'Indicadores', true, null, false); });
}

function Deletar(indicador) {
    
    $.ajax({ type: 'POST', url: '/Indicador/Delete', dataType: 'json', data: { registro: indicador.id } })
        .done(function (data) {
