...
function ListarTodos() {
    ...
    $.ajax({ type: "GET", url: "/CustomerView/GetAllCustomerView", dataType: "json" })
        .done(function (data) {

            _registros = data;

            var source = { datatype: "json", datafields: [{ name: 'id' }, { name: 'site' }, { name: 'businessUnit' }, { name: 'indicator' }, { name: 'customer' }, { name: 'year' }, { name: 'weekNumber' }, { name: 'result' }], localdata: _registros };
            var dataAdapter = new $.jqx.dataAdapter(source);
            var _c = tables.customerview;
            ...
            _jqxGrid = $("#jqxGrid").jqxGrid({
                ...
                columns: [
                            ...
                            button.click(function (event) {
                                ...
                                if (IndexGrid != undefined && IndexGrid > -1) {
                                    ...
                                    if (reg != undefined && reg != null) {
                                        ...
                                        $.ajax({ type: 'GET', url: `/CustomerView/Update?registro=${reg.id}`, contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
                    ...
                    {
                        text: _c.businessUnit, datafield: 'businessUnit', width: '200px', cellsalign: 'left'
                    },
                    {
                        text: _c.indicator, datafield: 'indicator', width: 'auto', cellsalign: 'left'
                    },
                    {
                        text: _c.customer, datafield: 'customer', width: '200px', cellsalign: 'left'
                    },
                    {
                        text: _c.year, datafield: 'year', width: '70px', cellsalign: 'right'
                    },
                    {
                        text: _c.weekNumber, datafield: 'weekNumber', width: '70px', cellsalign: 'right'
                    },
                    {
                        text: _c.result, datafield: 'result', width: '100px', cellsalign: 'right'
                    }
                ],
...
function Carregar() {
    $('#botaoCriar').on('click', function () {
        $.ajax({ type: 'GET', url: '/CustomerView/Create', contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
...
function Deletar(customerview) {

    $.ajax({ type: 'POST', url: '/CustomerView/Delete', dataType: 'json', data: { registro: customerview.id, } })
        .done(function (data) {
