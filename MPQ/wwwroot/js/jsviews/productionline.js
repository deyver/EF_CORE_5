...
function ListarTodos() {
    ...
    $.ajax({ type: "GET", url: "/ProductionLine/GetAllProductionLine", dataType: "json" })
        .done(function (data) {
        
        _registros = data;

        var source = { datatype: "json", datafields: [{ name: 'id' }, { name: 'name' }, { name: 'customer' }, { name: 'site' }, { name: 'area' }, { name: 'project' } ], localdata: _registros };
        var dataAdapter = new $.jqx.dataAdapter(source);
        var _c = tables.productionline;
        ...
        _jqxGrid = $("#jqxGrid").jqxGrid({
            ...
            columns: [
                        ...
                        button.click(function (event) {
                            ...
                            if (IndexGrid != undefined && IndexGrid > -1) {
                                ...
                                if (reg != undefined && reg != null) {
                                    ...
                                    $.ajax({ type: 'GET', url: `/ProductionLine/Update?registro=${reg.id}`, contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
                ...
                {
                    text: _c.area, datafield: 'area', width: '150px', cellsalign: 'center'
                },
                {
                    text: _c.customer, datafield: 'customer', width: '150px', cellsalign: 'center'
                },
                {
                    text: _c.project, datafield: 'project', width: '150px', cellsalign: 'center'
                }
            ],
...
function Carregar() {
    $('#botaoCriar').on('click', function () {
        $.ajax({ type: 'GET', url: '/ProductionLine/Create', contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
...
function Deletar(productionline) {

    $.ajax({ type: 'POST', url: '/ProductionLine/Delete', dataType: 'json', data: { registro: productionline.id, } })
        .done(function (data) {
