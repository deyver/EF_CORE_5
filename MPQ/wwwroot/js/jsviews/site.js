...
function ListarTodos() {
    ...
    $.ajax({ type: "GET", url: "/Site/GetAllSite", dataType: "json" })
        .done(function (data) {

            _registros = data;

            var source = { datatype: "json", datafields: [{ name: 'id' }, { name: 'name' }, { name: 'empresa' }], localdata: _registros };
            ...
            _jqxGrid = $("#jqxGrid").jqxGrid({
                ...
                columns: [
                            ...
                            button.click(function (event) {
                                ...
                                if (IndexGrid != undefined && IndexGrid > -1) {
                                    ...
                                    if (reg != undefined && reg != null) {
                                        ...
                                        $.ajax({ type: 'GET', url: `/Site/Update?registro=${reg.id}&dominio=${reg.login}`, contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
                    ...
                    {
                        text: _c.empresa, datafield: 'empresa', width: 'auto', cellsalign: 'left'
                    }
                ],
...
function Carregar() {
    $('#botaoCriar').on('click', function () {
        $.ajax({ type: 'GET', url: '/Site/Create', contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
    ...
    //Botao Exportar
    $("#btnExportar").click(function () { $("#jqxGrid").jqxGrid('exportdata', 'xls', 'Sites', true, null, false); });
}

function Deletar(site) {
    
    $.ajax({ type: 'POST', url: '/Site/Delete', dataType: 'json', data: { registro: site.id, dominio: site.name } })
        .done(function (data) {
