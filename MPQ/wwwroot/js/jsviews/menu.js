...
function ListarTodos() {
    ...
    $.ajax({ type: "GET", url: "/Menu/GetAllMenu", dataType: "json" })
        .done(function (data) {
        
            _registros = data;

            var source = { datatype: "json", datafields: [{ name: 'id' }, { name: 'level' }, { name: 'sequence' }, { name: 'parentId' }, { name: 'name' }, { name: 'title' }, { name: 'url' }, { name: 'iconUrl' }], localdata: _registros };
        ...
        _jqxGrid = $("#jqxGrid").jqxGrid({
            ...
            columns: [
                        ...
                        button.click(function (event) {
                            ...
                            if (IndexGrid != undefined && IndexGrid > -1) {
                                ...
                                if (reg != undefined && reg != null) {
                                    ...
                                    $.ajax({ type: 'GET', url: `/Menu/Update?registro=${reg.id}&dominio=${reg.login}`, contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
                ...
                {
                    text: _c.level, datafield: 'level', width: '100px', cellsalign: 'left'
                },
                {
                    text: _c.sequece, datafield: 'sequence', width: '100px', cellsalign: 'left'
                },
                {
                    text: _c.parentid, datafield: 'parentId', width: 'auto', cellsalign: 'left'
                },
                {
                    text: _c.name, datafield: 'name', width: 'auto', cellsalign: 'left'
                },
                {
                    text: _c.title, datafield: 'title', width: 'auto', cellsalign: 'left'
                },
                {
                    text: _c.url, datafield: 'url', width: 'auto', cellsalign: 'left'
                },
                {
                    text: _c.iconurl, datafield: 'iconUrl', width: '150px', cellsalign: 'left'
                }
            ],
...
function Carregar() {
    $('#botaoCriar').on('click', function () {
        $.ajax({ type: 'GET', url: '/Menu/Create', contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
    ...
    //Botao Exportar
    $("#btnExportar").click(function () { $("#jqxGrid").jqxGrid('exportdata', 'xls', 'Menus', true, null, false); });
}

function Deletar(menu) {
    $.ajax({ type: 'POST', url: '/Menu/Delete', dataType: 'json', data: { registro: menu.id } })
        .done(function (data) {
