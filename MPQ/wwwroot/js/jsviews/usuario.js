...
function ListarTodos() {
    ...
    $.ajax({ type: "GET", url: "/Usuario/GetAllUser", dataType: "json" })
        .done(function (data) {
        
        _registros = data;

        var source = { datatype: "json", datafields: [{ name: 'id' }, { name: 'login' }, { name: 'name' }, { name: 'defaultLanguage' }, { name: 'status' } ], localdata: _registros };
        var dataAdapter = new $.jqx.dataAdapter(source);
        var _c = tables.usuario;
        ...
        _jqxGrid = $("#jqxGrid").jqxGrid({
            ...
            columns: [
                        ...
                        button.click(function (event) {
                            ...
                            if (IndexGrid != undefined && IndexGrid > -1) {
                                ...
                                if (reg != undefined && reg != null) {
                                    ...
                                    $.ajax({ type: 'GET', url: `/Usuario/Update?registro=${reg.id}&dominio=${reg.login}`, contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
                ...
                //},
                {
                    text: _c.loginRede, datafield: 'login', width: '150px', cellsalign: 'left'
                },
                {
                    text: _c.nome, datafield: 'name', width: 'auto', cellsalign: 'left'
                },
                {
                    text: _c.idioma, datafield: 'defaultLanguage', width: '150px', cellsalign: 'center'
                },
                {
                    text: _c.status, datafield: 'status', width: '150px', cellsalign: 'center'
                }
            ],
...
function BuscarPlantasPorEmpresaSelecionada() {
    var idEmpresa = $('#IdEmpresaPadrao').val();
    $('#IdPlantaPadrao option').remove();

    $.ajax({
        type: 'GET',
        url: '/Usuario/ConsultaPlantasPorEmpresa?idEmpresa='+idEmpresa,
        beforeSend: function () {
            $('.preloader').fadeIn();
...
function Carregar() {
    $('#botaoCriar').on('click', function () {
        $.ajax({ type: 'GET', url: '/Usuario/Create', contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
    ...
    //Botao Exportar
    $("#btnExportar").click(function () { $("#jqxGrid").jqxGrid('exportdata', 'xls', 'Usuarios', true, null, false); });
}

function Deletar(usuario) {

    $.ajax({ type: 'POST', url: '/Usuario/Delete', dataType: 'json', data: { id: usuario.id, dominio: usuario.login } })
        .done(function (data) {
