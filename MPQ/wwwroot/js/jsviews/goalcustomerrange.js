...
function ListarTodos() {
    ...
    $.ajax({ type: "GET", url: "/GoalCustomerRange/GetAllGoalCustomerRange", dataType: "json" })
        .done(function (data) {

            _registros = data;

            var source = { datatype: "json", datafields: [{ name: 'id' }, { name: 'goalCustomer' }, { name: 'name' }, { name: 'value' }, { name: 'color' }, { name: 'operator' }], localdata: _registros };
            var dataAdapter = new $.jqx.dataAdapter(source);
            var _c = tables.goalcustomerrange;
            ...
            _jqxGrid = $("#jqxGrid").jqxGrid({
                ...
                columns: [
                            ...
                            button.click(function (event) {
                                ...
                                if (IndexGrid != undefined && IndexGrid > -1) {
                                    ...
                                    if (reg != undefined && reg != null) {
                                        ...
                                        $.ajax({ type: 'GET', url: `/GoalCustomerRange/Update?registro=${reg.id}`, contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
                    ...
                    {
                        text: _c.goalcustomer, datafield: 'goalCustomer', width: '200px', cellsalign: 'left'
                    },
                    {
...
function Carregar() {
    $('#botaoCriar').on('click', function () {
        $.ajax({ type: 'GET', url: '/GoalCustomerRange/Create', contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
...
function Deletar(goalcustomerrange) {

    $.ajax({ type: 'POST', url: '/GoalCustomerRange/Delete', dataType: 'json', data: { registro: goalcustomerrange.id, } })
        .done(function (data) {
