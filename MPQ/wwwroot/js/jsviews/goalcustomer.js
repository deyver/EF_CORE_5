...
function ListarTodos() {
    ...
    $.ajax({ type: "GET", url: "/GoalCustomer/GetAllGoalCustomer", dataType: "json" })
        .done(function (data) {

            _registros = data;

            var source = { datatype: "json", datafields: [{ name: 'id' }, { name: 'name' }, { name: 'site' }, { name: 'customer' }, { name: 'unitOfMeasurement' }, { name: 'stfNumber' }], localdata: _registros };
            var dataAdapter = new $.jqx.dataAdapter(source);
            var _c = tables.goalcustomer;
            ...
            _jqxGrid = $("#jqxGrid").jqxGrid({
                ...
                columns: [
                            ...
                            button.click(function (event) {
                                ...
                                if (IndexGrid != undefined && IndexGrid > -1) {
                                    ...
                                    if (reg != undefined && reg != null) {
                                        window.location.href = `/GoalCustomer/Edit/${reg.id}`;

                                        //$.ajax({ type: 'GET', url: `/GoalCustomer/Update?registro=${reg.id}`, contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
                    ...
                    {
                        text: _c.name, datafield: 'name', width: '200px', cellsalign: 'left'
                    },
                    {
                    ...
                    {
                        text: _c.unitofmeasurement, datafield: 'unitOfMeasurement', width: 'auto', cellsalign: 'left'
                    },
                    {
...
function Carregar() {
    $('#botaoCriar').on('click', function () {
        $.ajax({ type: 'GET', url: '/GoalCustomer/Create', contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
...
function Deletar(goalcustomer) {

    $.ajax({ type: 'POST', url: '/GoalCustomer/Delete', dataType: 'json', data: { registro: goalcustomer.id, } })
        .done(function (data) {
