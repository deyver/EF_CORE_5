...
function ListarTodos() {
    ...
    $.ajax({ type: "GET", url: "/Complaint/GetAllComplaint", dataType: "json" })
        .done(function (data) {

            _registros = data;

            var source = { datatype: "json", datafields: [{ name: 'id' }, { name: 'indicator' }, { name: 'complaintDate' }, { name: 'customer' }, { name: 'quantity' }, { name: 'issue' }, { name: 'containmentAction' }, { name: 'actionDate' }, { name: 'actionResponsible' }, { name: 'gqrsNumber' }, { name: 'summarySent' }, { name: 'site' }, { name: 'businessUnit' }], localdata: _registros };
            var dataAdapter = new $.jqx.dataAdapter(source);
            var _c = tables.complaint;
            ...
            _jqxGrid = $("#jqxGrid").jqxGrid({
                ...
                columns: [
                            ...
                            button.click(function (event) {
                                ...
                                if (IndexGrid != undefined && IndexGrid > -1) {
                                    ...
                                    if (reg != undefined && reg != null) {
                                        ...
                                        $.ajax({ type: 'GET', url: `/Complaint/Update?registro=${reg.id}`, contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
                    ...
                    {
                        text: _c.indicator, datafield: 'indicator', width: '150px', cellsalign: 'left'
                    },
                    {
                        text: _c.complaintDate, datafield: 'complaintDate', width: '100px', cellsalign: 'left'
                    },
                    {
                        text: _c.customer, datafield: 'customer', width: '200px', cellsalign: 'left'
                    },
                    {
                        text: _c.quantity, datafield: 'quantity', width: '100px', cellsalign: 'right'
                    },
                    {
                        text: _c.issue, datafield: 'issue', width: 'auto', cellsalign: 'left'
                    },
                    {
                        text: _c.gqrsNumber, datafield: 'gqrsNumber', width: '100px', cellsalign: 'right'
                    },
                    {
                        text: _c.summarySent, datafield: 'summarySent', width: '150px', cellsalign: 'center'
                    }
                ],
...
function Carregar() {
    $('#botaoCriar').on('click', function () {
        $.ajax({ type: 'GET', url: '/Complaint/Create', contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
...
function Deletar(complaint) {

    $.ajax({ type: 'POST', url: '/Complaint/Delete', dataType: 'json', data: { registro: complaint.id, } })
        .done(function (data) {
