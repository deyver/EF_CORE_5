...
function ListarTodos() {
    ...
    $.ajax({ type: "GET", url: "/Customer/GetAllCustomer", dataType: "json" })
        .done(function (data) {
        ...
        _jqxGrid = $("#jqxGrid").jqxGrid({
            ...
            columns: [
                        ...
                        button.click(function (event) {
                            ...
                            if (IndexGrid != undefined && IndexGrid > -1) {
                                ...
                                if (reg != undefined && reg != null) {
                                    ...
                                    $.ajax({ type: 'GET', url: `/Customer/Update?id=${reg.id}`, contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
                ...
                {
                    text: _c.name, datafield: 'name', width: 'auto'
                }
            ],
...
function Carregar() {
    ...
    $('#botaoCriar').on('click', function () {
        $.ajax({ type: 'GET', url: '/Customer/Create', contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
    ...
    //Botao Exportar
    $("#btnExportar").click(function () { $("#jqxGrid").jqxGrid('exportdata', 'xls', 'Customer', true, null, false); });
}

function Deletar(customer) {
    
    $.ajax({ type: 'POST', url: '/Customer/Delete', dataType: 'json', data: { id: customer.id } })
        .done(function (data) {
