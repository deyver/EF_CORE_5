...
function ListarTodos() {
    ...
    $.ajax({ type: "GET", url: "/GoalProductionLineRange/GetAllGoalProductionLineRange", dataType: "json" })
        .done(function (data) {

            _registros = data;

            var source = { datatype: "json", datafields: [{ name: 'id' }, { name: 'productionLine' }, { name: 'name' }, { name: 'value' }, { name: 'color' }, { name: 'operator' }], localdata: _registros };
            var dataAdapter = new $.jqx.dataAdapter(source);
            var _c = tables.goalproductionlinerange;
            var cellsrenderer = function (row, columnfield, value, defaulthtml, columnproperties) { return '<div class="jqx-grid-cell-middle-align" style="margin-top: 17px; background-color: ' + value + ';">' + value + '</div>'; }
            ...
            _jqxGrid = $("#jqxGrid").jqxGrid({
                ...
                columns: [
                            ...
                            button.click(function (event) {
                                ...
                                if (IndexGrid != undefined && IndexGrid > -1) {
                                    ...
                                    if (reg != undefined && reg != null) {
                                        ...
                                        $.ajax({ type: 'GET', url: `/GoalProductionLineRange/Update?registro=${reg.id}`, contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
                    ...
                    {
                        text: _c.productionline, datafield: 'productionLine', width: '200px', cellsalign: 'left'
                    },
                    {
                        text: _c.name, datafield: 'name', width: 'auto', cellsalign: 'left'
                    },
                    {
                        text: _c.value, datafield: 'value', width: '200px', cellsalign: 'left'
                    },
                    {
                        text: _c.color, datafield: 'color', width: '200px', cellsalign: 'left', cellsrenderer: cellsrenderer
                    },
                    {
                        text: _c.operator, datafield: 'operator', width: '200px', cellsalign: 'left'
                    }
                ],
...
function Carregar() {
    $('#botaoCriar').on('click', function () {
        $.ajax({ type: 'GET', url: '/GoalProductionLineRange/Create', contentType: 'html', beforeSend: function () { $('.preloader').fadeIn(); } })
...
function Deletar(goalproductionlinerange) {

    $.ajax({ type: 'POST', url: '/GoalProductionLineRange/Delete', dataType: 'json', data: { registro: goalproductionlinerange.id, } })
        .done(function (data) {
